...
use crate::{constants::{auth_email_string, auth_password_string}, handlers::auth::models::SignInMessage};
...
pub fn check_is_valid_session(session : &Session) -> Result<SignInMessage, Error>
{
    ...
    if let (Ok(Some(email_value)), Ok(Some(password_value))) = (email, password) {
        Ok(SignInMessage { email : email_value, password : password_value })
    }
    else {
